classes_focus= {'a person riding a bicycle':["person", "bicycle"],
                'a certain number of persons fighting':["person"],
                'a group of persons playing': ["person","frisbee", "sports ball","baseball glove","tennis racket"],
...
def eventsCheck(event, classes,  detections, results, frames, MLLM):
    match event:
        ...
        case 'a person running':
            condition, prompt =Check_Running(classes, detections, results, frames, MLLM)
...
def Check_Playing(classes, detections):
    ...
    if any([classes[class_] > 0 for class_ in classes_of_interest]) and classes["person"] > 1:
        return True, 'yes'
    else: 
        return False, 'no'
...
def person_lying(loaded_data):
    ...
    for i in range(len(loaded_data[0])):
        print('To evalute:',loaded_data[0][i], '\n---------------------------------------------------------------------\n')
        contador = 0
        ...
        for j in range(1, len(loaded_data)):
            ...
            for k in range(len(loaded_data[j])):
                ...
                if calculate_shared_area(loaded_data[0][i], loaded_data[j][k]) > 0.93:
                    if stored_data == []:
                        stored_data.append([loaded_data[j][k], calculate_shared_area(loaded_data[0][i], loaded_data[j][k])])
                    else:
                        if stored_data[0][1] < calculate_shared_area(loaded_data[0][i], loaded_data[j][k]):
                            stored_data[0] = [loaded_data[j][k], calculate_shared_area(loaded_data[0][i], loaded_data[j][k])]
...
def person_running(loaded_data):
    ...
    for i in range(len(loaded_data[0])):
        ...
        for j in range(1, len(loaded_data)):
            ...
            for k in range(len(loaded_data[j])):
                print(j, '-',k,'-',loaded_data[0][i],loaded_data[j][k], calculate_shared_area(loaded_data[0][i], loaded_data[j][k]),'\n')
                if calculate_shared_area(loaded_data[0][i], loaded_data[j][k]) > 0.2 and calculate_shared_area(loaded_data[0][i], loaded_data[j][k]) < 0.9:
                    if stored_data == []:
                        stored_data.append([loaded_data[j][k], calculate_shared_area(loaded_data[0][i], loaded_data[j][k])])
                    else:
                        if stored_data[0][1] < calculate_shared_area(loaded_data[0][i], loaded_data[j][k]):
