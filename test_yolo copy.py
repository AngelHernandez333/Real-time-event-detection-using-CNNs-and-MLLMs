...
while True:
    ...
    #results=ov_model(img, stream=False)
    #results = ov_qmodel.predict(source=img, device=device)
    for r in results:
        boxes= r.boxes
        for box in boxes:
    ...
    time_values = np.append(time_values, [[elapsed_time]], axis=0)
    if cv2.waitKey(1)==ord('q') or i==1000:
        break
cap.release()
cv2.destroyAllWindows()
print(time_values)
print(np.mean(time_values)*1000)