...
def decision_makerComplex(frame, frame_number, frames, gap, classes, detections, event, results, MLLM=True,frames_number=[],prompts=[]):
    if frame_number % gap == 0:
        condition, prompt=eventsCheck( event, classes ,detections,results, frames, MLLM)
