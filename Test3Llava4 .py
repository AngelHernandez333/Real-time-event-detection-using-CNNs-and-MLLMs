...
from Functions3 import take_frame, prompt_text, decision_makerComplex,check_precision,  detection_labels
import pandas as pd
import os
...
def testing(video_path, event ,end, detector_usage):
    # Clases de detección
    ...
    while True:
        ...
        if detector_usage==0:
            ...
            detections=detection(frame, classes, ov_qmodel)
            decision_makerComplex(frame, int(cap.get(cv2.CAP_PROP_POS_FRAMES)), frames, 5 , classes, detections, event, results, False, frames_number,prompts)
        if len(frames) > 6 and detector_usage<4:
            frames.pop(0)
            results.pop(0)
            frames_number.append(int(cap.get(cv2.CAP_PROP_POS_FRAMES)))
            validation(frames, classes, processor, model, prompts, event, detector_usage)
        # -------------------------------------
    ...
    return frames_number, fps_list, prompts, duration, time_video
...
if __name__ == "__main__":
    ...
    columns = ['Name', 'Mode','Precision', 'Recall', 'Event', 'Validations Number', 'Duration', 'Process time']  
    '''
    video_kind=3
    video_number=0
    i=0
    ...
    prompts=prompts[1::]
    print('Prompts:', prompts, 'Frames numbers',frames_number,)
    ...
    for video_kind in range(len(videos)):
        for video_number in range(len(videos[video_kind])):
            for i in range(4):
                frames_number, fps_list, prompts, duration, time_video =testing(f"../Database/CHAD DATABASE/{events[video_kind]}/{videos[video_kind][video_number]}", description[video_kind], 0,i )   
                frames_number=frames_number[1::]
                prompts=prompts[1::]
                ...
                #Save the results
                row = {
                'Name': videos[video_kind][video_number], 'Mode': i,'Precision':precision, 'Recall':recall, 'Event':description[video_kind], 'Validations Number':len(prompts), 'Duration': duration, 'Process time': time_video}
                # Append the row to the DataFrame
                df = pd.concat([df, pd.DataFrame([row])], ignore_index=True)
                #fps_list = np.array(fps_list)
                print('\n', df)
    df.to_csv('Results/results_v2.csv', index=False) 
    print('\n', df)