...
if __name__ == "__main__":
    ...
    for video_kind in range(len(videos)):
        for video_number in range(len(videos[video_kind])):
            for i in range(5):
                frames_number, fps_list, prompts, duration, time_video = testing(
                    f"../Database/CHAD DATABASE/{events[video_kind]}/{videos[video_kind][video_number]}",
                ...
                precision, recall = check_precision(
                    prompts, frames_number, videos[video_kind][video_number]
                )
                # Save the results
                row = {
                    "Name": videos[video_kind][video_number],
                    "Mode": i,
                    "Precision": precision,
                    "Recall": recall,
