...
def get_dst_points(theta_deg, w, h, f):
    ...
    br_y = bl_y
    return np.array(
        [[tl_x, tl_y], [tr_x, tr_y], [br_x, br_y], [bl_x, bl_y]], dtype=np.float32
    )
...
def stacked_image(frames, tilt_increment, offset_x, offset_y):
    ...
    for i, (img, min_x, min_y, new_w, new_h) in enumerate(transformed_images):
        ...
        large_canvas.paste(
            img_pil, (int(i * offset_x - min_x), int(i * offset_y - min_y)), img_pil
        )

    # Save the final image
    large_canvas.save("/home/ubuntu/Tesis/stacked_image_max_um2.png")


if __name__ == "__main__":
    # Parameters
    tilt_increment = 0  # Degrees to tilt each frame
    offset_x = 120 * 3  # Horizontal offset for stacking
    offset_y = 30 * 12  # Vertical offset for stacking
    path = "/home/ubuntu/Database/ALL/Videos/Fighting/2_079_1.mp4"  # Replace with your video file path
    frame_final = 385
    frames = []
    step=5
    number_of_frames=6
    selection=[x for x in range(frame_final-number_of_frames*step,frame_final,step)]
    cap = cv2.VideoCapture(path)
    while True:
        ret, frame = cap.read()
        if not ret:
            break
        if int(cap.get(cv2.CAP_PROP_POS_FRAMES)) in selection:
            frames.append(frame)
